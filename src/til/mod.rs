...
impl Basic {
    fn new(til: &TILSectionHeader, bt: u8, btmt: u8) -> Result<Self> {
        ...
        match bt {
            ...
            bt_int @ BT_INT8..=BT_INT => {
                let is_signed = match btmt {
                    ...
                    BTMT_CHAR => match bt_int {
                        ...
                        _ => return Err(anyhow!("Reserved use of tf_int::BTMT_CHAR {:x}", btmt)),
                    },
                    _ => unreachable!(),
                };
            ...
            _ => Err(anyhow!("Unkown Unset Type {}", btmt)),
        }
    }
}
...
impl PointerRaw {
    fn read<I: BufRead>(input: &mut I, header: &TILSectionHeader, metadata: u8) -> Result<Self> {
        use flag::tf_ptr::*;
...
impl FunctionRaw {
    fn read<I: BufRead>(input: &mut I, header: &TILSectionHeader, metadata: u8) -> Result<Self> {
        ...
        // TODO double check ducumentation for [flag::tf_func::BT_FUN]
...
impl ArrayRaw {
    fn read<I: BufRead>(input: &mut I, header: &TILSectionHeader, metadata: u8) -> Result<Self> {
        ...
        let (base, nelem) = match metadata {
            ...
            // I think is only for zero, but ducumentation says anything other then BTMT_NONBASED
            _ => {
    ...
    fn read<I: BufRead>(input: &mut I) -> Result<Self> {
        ...
        match &buf[..] {
            [b'#', data @ ..] => {
                ...
                if tmp.position() != data.len().try_into().unwrap() {
...
impl Bitfield {
    fn read<I: BufRead>(input: &mut I, metadata: u8) -> Result<Self> {
