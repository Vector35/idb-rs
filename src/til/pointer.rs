...
impl Pointer {
    pub(crate) fn read(
        ...
        let closure = match metadata {
            BTMT_DEFPTR => PointerType::Default,
            BTMT_CLOSURE => PointerType::read(&mut *input, header)?,
            // TODO find the meaning of this
            BTMT_FAR => PointerType::Far,
            BTMT_NEAR => PointerType::Near,
            _ => unreachable!(),
        };
        ...
        let (_ta_lower, is_shifted, ptr_type_raw) = match input.read_tah()? {
            None => return Err(anyhow!("Unexpected EoF on Pointer TAH value")),
            Some(None) => (0, false, 0),
            Some(Some(TypeAttribute {
                tattr,
                extended: _extended,
            })) => {
        ...
        let typ = Type::read(&mut *input, header, None, fields, comments)?;
