...
use crate::til::{flag, TILMacro, TILTypeInfo, TyperefValue};
...
impl TILSection {
    pub fn decompress<I: IdbBufRead, O: Write>(
        ...
        let mut header = TILSection::read_header(&mut input)?;
    ...
    fn decompress_bucket(
        ...
        let (ndefs, len, compressed_len) =
            TILSection::read_bucket_zip_header(&mut *input)?;
    ...
    pub fn get_ref_value_idx(&self, ref_value: &TyperefValue) -> Option<usize> {
        match ref_value {
            TyperefValue::Name(name) => {
                self.get_name_idx(name.as_ref()?.as_bytes())
            }
            TyperefValue::Ordinal(ord) => self.get_ord_idx((*ord).into()),
        }
    }
...
impl TILSection {
    pub fn read(input: &mut impl IdbBufRead) -> Result<TILSection> {
        ...
        let types = Self::read_bucket(&mut *input, &header, next_ordinal)
            .context("Parsing TIL Types")?;
        let macros = header
            .flags
            .has_macro_table()
            .then(|| {
                Self::read_macros(&mut *input, &header)
                    .context("Parsing TIL Macros")
            })
            .transpose()?;
    ...
    fn read_bucket_inner(
        ...
        let type_info_raw: Vec<_> = (0..ndefs)
            .map(|i| {
                TILTypeInfo::read(&mut input, header)
                    .with_context(|| format!("TIL Bucket member {i}"))
            })
            .collect::<Result<_>>()?;
        #[cfg(feature = "restrictive")]
        ensure!(
