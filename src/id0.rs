...
impl ID0Section {
    ...
    fn read_inner<I: Read>(input: &mut I, idb_header: &IDBHeader) -> Result<Self> {
        ...
        let mut pages = Vec::with_capacity(header.page_count.try_into()?);
        loop {
        ...
        let mut entries = Vec::with_capacity(header.record_count.try_into()?);
    ...
    fn create_tree(
        ...
        let index = usize::try_from(index.get())? - 1;
        let entry = pages
            .get_mut(index)
...
impl ID0TreeEntrRaw {
    ...
    fn read_xx(
        ...
        input.seek(SeekFrom::Start(data_offsets.next().unwrap().into()))?;
